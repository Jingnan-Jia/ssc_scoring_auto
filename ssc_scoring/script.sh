...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run.py 2>${slurm_dir}/slurm-${job_id}_$idx.err 1>${slurm_dir}/slurm-${job_id}_$idx.out --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname=$(hostname) --epochs=501 --net='convnext_tiny' --sys=1 --sampler=1 --pretrained=1 --sys_pro_in_0=0.5 --sys_ratio=0.0 --mode='train' --weighted_syn_region=1 --batch_size=10 --fold=1 --gen_gg_as_retp=1 --remark="balance sampler+sys+pretrained, weight_map (switch x,y), 16 patients in test dataset, interval=5, with pred outpout"
