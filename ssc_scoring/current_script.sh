...
idx=0; export CUDA_VISIBLE_DEVICES=$idx; stdbuf -oL python -u run_pos.py 2>${slurm_dir}/slurm-${job_id}_$idx.err 1>${slurm_dir}/slurm-${job_id}_$idx.out --outfile=${slurm_dir}/slurm-${job_id}_$idx --hostname="$(hostname)" --net="vgg16_3d" --train_on_level=0 --batch_size=1 --mode='train' --infer_2nd=0 --eval_id=0 --level_node=0 --fold=1 --remark="vgg16, train/testing dataset is the same as PFT"
