...
if __name__ == '__main__':
    ...
    while nb_img < 10:
        ...
        data = next(test_dataloader)  # A list of dict
        xs, ys = data['image_key'], data['label_key']  # [batch, channel, w, h, d]
        idx = 0
        for x_, y_ in zip(xs, ys):  # x, y shape: [channel, w, h, d]
            idx += 1
            print(f'idx, {idx}')
            if idx % 3 == 0:  # skip next 2 images because the neighboring 3 images are similar (up, middl, down)
                print(f'idx is okay, {idx}')
                x, y = x_[None], y_[None]  # x, y shape: [batch, channel, w, h, d]
