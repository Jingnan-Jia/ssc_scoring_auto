...
for scan in scan_files[100:]:
    ...
    ct_dia = morphology.binary_dilation(ct_neg, np.ones((6,6,6))).astype(int)
    ct_lung = largest_connected_parts(ct_dia, 2)
    ct_ero = morphology.binary_erosion(ct_lung, np.ones((6,6,6))).astype(int)

    save_itk(scan.split('.mha')[0] + '_lung.mha', ct_ero, ori, sp)
